...
#   timestamp: 2024-03-07T09:30:27+00:00
...
class CycleEventNameEnum(BaseModel):
    __root__: Any = Field(..., title='CycleEventNameEnum')


class EventDrivenNameEnum(BaseModel):
    __root__: Any = Field(..., title='EventDrivenNameEnum')
...
class CycleEventSchema(BaseModel):
    event_name: Any = Field(
        ...,
        description='事件名称，比如 InventoryKpiEvent, 事件唯一标识, enum类型从配置文件中加载',
        title='CycleEventNameEnum',
    )
    event_type: Optional[EventType] = Field(
        'register', description='事件类型，分为注册和取消'
    )
    callback_url: str = Field(..., description='事件回调地址', title='Callback Url')
...
class EventDrivenSchema(BaseModel):
    event_name: Any = Field(
        ...,
        description='事件唯一标识，比如 container_update，enum类型从配置文件中加载 ',
        title='EventDrivenNameEnum',
    )
