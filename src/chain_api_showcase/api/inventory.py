...
class InventoryRequest(object):
    ...
    async def AddPoint(cls, body: AddPointSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/position/AddPoint, response: {resp.text}")
        return resp.status, resp.json()
        
    @classmethod
    async def GetPointsHeading(cls, body: PositionWithoutTierSchema) -> Tuple[int, Dict]:
        """
        获取点位Heading
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/position/GetPointsHeading, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def GetAvailableTier(cls, body: GetAvailableTierSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/position/GetAvailableTier, response: {resp.text}")
        return resp.status, resp.json()
        
    @classmethod
    async def GetContainerPoints(cls, body: GetContainerPointsSchema) -> Tuple[int, Dict]:
        """
        查询箱子和点位
        Api Description
        """
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/GetContainerPoints, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def ContainerCheck(cls, body: ContainerCheckSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/ContainerCheck, response: {resp.text}")
        return resp.status, resp.json()
        
    @classmethod
    async def ImportAreaContainers(cls, body: ImportContainersSchema) -> Tuple[int, Dict]:
        """
        批量导入固定区域箱子
        对某个区域进行批量导箱
        """
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/ImportAreaContainers, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def ImportContainers(cls, body: ImportContainersSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/ImportContainers, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def RemoveContainers(cls, body: RemoveContainerSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/RemoveContainers, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def LimitLow(cls, body: LimitLowSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/LimitLow, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def ContainerQuery(cls, body: ContainerQuerySchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/ContainerQuery, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def ContainerTaskCheck(cls, body: TaskCheckSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/ContainerTaskCheck, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def CommonInfo(cls, body: ContainerCommonInfoSchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/container/CommonInfo, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def TGInitNotify(cls, body: TGInitNotifySchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/TGInitNotify, response: {resp.text}")
        return resp.status, resp.json()
    ...
    async def QcLeaveNotify(cls, body: QcLeaveNotifySchema) -> Tuple[int, Dict]:
        ...
        if resp.status != 200:
            print(f"Request failed: {resp.status}, url: {cls.url}, api: /api/QcLeaveNotify, response: {resp.text}")
        return resp.status, resp.json()
